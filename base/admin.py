...
class DataTypeCheckboxSelectMultiple(forms.CheckboxSelectMultiple):
    ...
    def create_option(self, name, value, label, selected, index, subindex=None, attrs=None):
        ...
        if value and self.provider_type:
            ...
            from ingestors.provider_mappings import get_data_type_config
...
class ProviderAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.provider_type:
            ...
            self.fields[
                "excluded_data_types"
            ].help_text = "Save the provider first, then you can configure which data types to exclude."
            self.fields["excluded_data_types"].widget = forms.TextInput(
                attrs={"readonly": True, "placeholder": "Save provider first to configure data types"}
            )
    ...
    def clean_excluded_data_types(self):
        ...
        from ingestors.provider_mappings import Provider as ProviderEnum
        from ingestors.provider_mappings import validate_data_types
    ...
    def clean(self):
        ...
        if self.errors:
            ...
            raise ValidationError(f"Please fix the following errors: {'; '.join(error_messages)}")
...
class ProviderAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Basic Information", {"fields": ("name", "provider_type", "active")}),
        (
        ...
        ("Advanced", {"fields": ("credentials",), "classes": ("collapse",)}),
    )
