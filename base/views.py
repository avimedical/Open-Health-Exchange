...
from base.serializers import (
    ProviderSerializer, ProviderLinkSerializer, ProviderLinkingRequestSerializer,
    SyncStatusSerializer, HealthDataCapabilitiesSerializer, DeviceSyncRequestSerializer,
...
class HealthSyncViewSet(viewsets.ViewSet):
    ...
    def status(self, request):
        ...
        if not ehr_user_id:
            return Response(
                {'error': 'ehr_user_id parameter is required'},
        ...
        try:
            ...
            return Response(
                {'error': f'User {ehr_user_id} not found'},
        ...
        if provider:
            ...
            sync_status = cache.get(cache_key, {
            ...
            for prov in providers:
                ...
                prov_status = cache.get(cache_key, {
                    'status': 'no_recent_sync',
                    'last_sync': None,
                    'records_synced': None,
                    'errors': []
    ...
    def providers(self, request):
        ...
        return Response({
            'ehr_user_id': ehr_user_id,
            'connected_providers': serializer.data,
    ...
    def trigger_device_sync(self, request):
        ...
        try:
            ...
            response_data = {
                ...
                'sync_id': f"device-sync-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
                'ehr_user_id': ehr_user_id,
                'provider': provider,
                'devices_processed': result.processed_devices,
                'associations_created': result.processed_associations,
            ...
            return Response(
                {'error': 'Device sync failed', 'message': str(e)},
...
def trigger_device_sync(request, provider):
    ...
    try:
        ...
        if not provider_link:
            return Response({
                'error': f'No active {provider} provider link found for user {ehr_user_id}'
            }, status=404)
        ...
        try:
            ...
            if not access_token:
                return Response({
                    'error': f'No access token found for {provider} provider link'
                }, status=400)
            ...
            return Response({
                'error': f'Failed to queue device sync: {str(e)}'
            }, status=500)
