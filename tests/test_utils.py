...
class TestMetrics:
    ...
    def operation(self, operation_name: str, status: str = "✅"):
        ...
        self.operations.append({"operation": operation_name, "status": status})
...
class TestDataFactory:
    ...
    def create_date_range():
        ...
        return DateRange(start=datetime(2024, 9, 1, tzinfo=UTC), end=datetime(2024, 9, 25, tzinfo=UTC))
...
def assert_cache_key_format(cache_key: str, expected_prefix: str, expected_parts: int = None):
    ...
    if expected_parts:
        ...
        assert len(parts) == expected_parts, (
            f"Cache key should have {expected_parts} parts, got {len(parts)}: {cache_key}"
        )
...
def assert_dataclass_immutable(dataclass_instance):
    ...
    try:
        if hasattr(dataclass_instance, "__dataclass_fields__"):
            ...
            raise AssertionError("Dataclass should be frozen but allowed modification")
