...
class WebhookSignatureValidator:
    ...
    def validate_withings_signature(self, request: HttpRequest) -> bool:
        ...
        try:
            ...
            if not is_valid:
                logger.warning(
                    f"Invalid Withings webhook signature. Expected: {expected_signature[:8]}..., Got: {provided_signature[:8]}..."
                )
    ...
    def validate_fitbit_signature(self, request: HttpRequest) -> bool:
        ...
        try:
            ...
            expected_signature = base64.b64encode(
                hmac.new(client_secret.encode("utf-8"), body, hashlib.sha1).digest()
            ).decode("utf-8")
            ...
            if not is_valid:
                logger.warning(
                    f"Invalid Fitbit webhook signature. Expected: {expected_signature[:8]}..., Got: {signature_header[:8]}..."
                )
    ...
    def validate_generic_signature(
        ...
        try:
            ...
            # Calculate expected signature
            body = request.body
            expected_signature = hmac.new(secret.encode("utf-8"), body, hash_algorithm).hexdigest()
            ...
            if not is_valid:
                logger.warning(
                    f"Invalid webhook signature. Expected: {expected_signature[:8]}..., Got: {provided_signature[:8]}..."
                )
    ...
    def validate_bearer_token(self, request: HttpRequest, expected_token: str | None = None) -> bool:
        """
    ...
    def is_signature_validation_enabled(self) -> bool:
        ...
        return (
            hasattr(settings, "WITHINGS_WEBHOOK_SECRET")
            or hasattr(settings, "SOCIAL_AUTH_FITBIT_SECRET")
            or getattr(settings, "WEBHOOK_SIGNATURE_VALIDATION_ENABLED", True)
        )
