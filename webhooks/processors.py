...
from ingestors.health_data_constants import Provider, HealthDataType
...
class WebhookPayloadProcessor:
    ...
    def process_withings_webhook(self, payload: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if 'startdate' in payload and 'enddate' in payload:
                try:
                    start_time = datetime.fromtimestamp(int(payload['startdate']))
                    end_time = datetime.fromtimestamp(int(payload['enddate']))
                    date_range = {
            ...
            logger.info(f"Processed Withings webhook for user {user_id}, appli {appli}, data types: {[dt.value for dt in data_types]}")
            return [sync_request]
            ...
            for notification in payload:
                try:
                    ...
                    try:
                        ...
                        start_time = sync_date
                        end_time = sync_date + timedelta(days=1)
                    ...
                    logger.info(f"Processed Fitbit webhook notification for user {user_id}, collection {collection_type}, data types: {[dt.value for dt in data_types]}")
    ...
    def process_generic_webhook(self, payload: Dict[str, Any], provider: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Processed generic webhook for provider {provider}, user {user_id}, data types: {valid_data_types}")
            return [sync_request]
