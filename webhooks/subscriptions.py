...
class WebhookSubscriptionManager:
    ...
    def create_withings_subscription(self, user_id: str, data_types: list[str] | None = None) -> WebhookSubscription:
        """
        ...
        try:
            ...
            failed_appli_types = []
            logger.info(
                f"Attempting to create Withings subscriptions for user {user_id} with appli types: {appli_types}"
            )
            ...
            for appli in appli_types:
                ...
                try:
                    ...
                    logger.info(
                        f"Withings API response for appli {appli}: status={result.get('status')}, body={result}"
                    )
                    ...
                    if result.get("status") == 0:  # Withings returns 0 for success
                        ...
                        logger.error(
                            f"✗ Withings subscription failed for appli {appli}: status={result.get('status')}, error={result.get('error', 'Unknown error')}"
                        )
            ...
            logger.info(
                f"Withings subscription summary for user {user_id}: {success_count} succeeded, {failed_count} failed"
            )
            ...
            if not created_subscriptions:
                raise WebhookSubscriptionError(
                    f"Failed to create any Withings subscriptions. Attempted appli types: {appli_types}"
                )
    ...
    def create_fitbit_subscription(
        self, user_id: str, subscription_id: str | None = None, collection_types: list[str] | None = None
    ) -> WebhookSubscription:
        """
            ...
            for collection_type in collections:
                ...
                try:
                    ...
                    if response.status_code == 201:  # Created
                        ...
                        logger.info(
                            f"Fitbit subscription already exists for user {user_id}, collection {collection_type}"
                        )
                        subscription = WebhookSubscription(
                            provider=Provider.FITBIT,
                            user_id=user_id,
    ...
    def delete_fitbit_subscription(
        self, user_id: str, subscription_id: str, collection_type: str = "activities"
    ) -> bool:
