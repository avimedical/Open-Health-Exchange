    ...
    def _queue_sync_tasks(self, provider: Provider, sync_requests: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for sync_request in sync_requests:
            try:
                ...
                self.logger.info(
                    f"Queued {provider.value} sync task {task_result.id} for user {sync_request['user_id']}"
                )
...
def webhook_health_check(request):
    ...
    response_data = {"status": "healthy", "timestamp": timezone.now().isoformat(), "webhook_system": stats}
...
def webhook_metrics_endpoint(request):
    ...
    response_data = {"webhook_metrics": stats, "timestamp": timezone.now().isoformat()}
